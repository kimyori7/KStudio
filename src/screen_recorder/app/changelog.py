...
CHANGELOG: list[tuple[str, list[str]]] = [
    ("1.0.7", [
        "영상 편집: 클립을 복사해 다른 영상에 붙여넣을 수 있습니다 — 자른 조각을 클릭하고 Ctrl+C, 다른 영상 탭으로 가서 Ctrl+V 하면 재생 위치에 붙습니다 (트랙 우클릭 메뉴로도 가능).",
        "영상 편집: 파일 ▸ 새 영상 프로젝트(Ctrl+Shift+N) 추가 — 빈 타임라인에서 시작해 여러 영상의 조각을 모아 하나로 이어 붙일 수 있습니다.",
        "영상 편집: 클립 안에 들어 있던 캡션·효과도 클립과 함께 복사됩니다. 붙여넣기는 Ctrl+Z 한 번으로 되돌아갑니다.",
        "영상 편집: 길이를 읽을 수 없는 영상 파일을 트랙에 놓으면 조용히 무시하지 않고 알려줍니다 (보이지 않는 빈 클립이 생기던 문제).",
    ]),
    ("1.0.6", [
        "문서 모드: 파일이 외부에서 바뀌었는데 갱신 팝업이 안 뜨던 문제를 고쳤습니다 — 이제 파일을 주기적으로 직접 확인해서, 윈도우의 변경 알림을 놓쳐도 잡아냅니다.",
        "문서 모드: 갱신 팝업이 뜰 때 KStudio 창을 앞으로 가져옵니다 (다른 프로그램 뒤에 깔려 못 보던 문제).",
        "문서 모드: 미리보기에서 드래그로 영역을 지정할 때 화면이 앞서 선택했던 위치로 튀거나 저절로 스크롤되던 문제를 고쳤습니다.",
    ]),
    ("1.0.4", [
        "라이브러리: 파일을 열면 그 항목이 맨 위로 올라갑니다 — 이미 라이브러리에 있어도 (영상·이미지·문서 공통).",
        "라이브러리: 항목을 드래그해서 순서를 바꿀 수 있습니다. 바뀐 순서는 재시작 후에도 유지됩니다.",
    ]),
    ("1.0.3", [
        "문서 모드: ⟳ 새로고침 버튼 추가(비교 옆) — 외부 변경이 반영 안 될 때 한 번에 최신 내용을 불러오고 미리보기를 다시 그립니다.",
        "문서 모드: 파일로 연 문서는 미리보기 화면으로 열립니다 (새 문서는 기존처럼 나란히).",
        "문서 모드: 외부 변경 감지가 파일 잠금 등으로 놓치던 경우 재시도하고, 미리보기가 하얗게 죽으면 자동 복구합니다.",
        "앱이 비정상 종료된 직후 다시 실행하면 창이 안 뜨던 문제를 고쳤습니다.",
    ]),
    ("1.0.2", [
        "문서 모드: 미리보기의 하이퍼링크가 동작합니다 — 외부 링크는 브라우저로 열리고, 문서 내 앵커(#섹션)는 해당 위치로 이동합니다.",
        "문서 모드: 미리보기 우클릭 메뉴를 복사·링크 주소 복사·모두 선택으로 정리했습니다 (Reload로 미리보기가 사라지던 문제 제거).",
    ]),
    ("1.0.1", [
