"""스크린샷 설정 패널 — 저장 폴더, 파일명 패턴 + 저장된 스크린샷 리스트."""
...
from ...core.settings import ScreenshotSettings
...
class ScreenshotPanel(QWidget):
    ...
    def __init__(self, settings: ScreenshotSettings):
        ...
        header_row.addWidget(QLabel("📸 <b>스크린샷 파일</b>"))
    ...
    def _browse(self) -> None:
        path = QFileDialog.getExistingDirectory(self, "저장 폴더", self.dir_edit.text())
        if path:
            self.dir_edit.setText(path)
            self._sync()

    def _sync(self) -> None:
        old = self.settings.save_dir
        self.settings.save_dir = self.dir_edit.text()
        self.settings.filename_pattern = self.pattern_edit.text()
        self.settings_changed.emit()
        if self.settings.save_dir != old:
            self._rewatch()
