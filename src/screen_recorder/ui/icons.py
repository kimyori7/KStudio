...
_PATHS: dict[str, str] = {
    ...
    # 이미지 생성 도구 액션용 — lucide "image-plus" (2026-05-27).
    "image-plus": (
    ...
    # 폰트 크기 줄이기 — 대문자 "A"(좌) + 빼기(우). Markdown 문서 폰트 컨트롤(Phase 55.5).
    "font-decrease": (
        '<path d="M5 18 L9 6 L13 18"/>'      # A 외곽
        '<path d="M6.3 14 H11.7"/>'          # A 가로획
        '<path d="M15.5 13 H21.5"/>'         # 빼기
    ),
    # 폰트 크기 키우기 — 대문자 "A"(좌) + 더하기(우).
    "font-increase": (
        '<path d="M5 18 L9 6 L13 18"/>'      # A 외곽
        '<path d="M6.3 14 H11.7"/>'          # A 가로획
        '<path d="M15.5 13 H21.5"/>'         # 더하기 가로
        '<path d="M18.5 10 V16"/>'           # 더하기 세로
    ),
    # 되돌리기(기본값) — 반시계 원형 화살표. lucide rotate-ccw.
    "rotate-ccw": (
        '<path d="M3 2v6h6"/>'
