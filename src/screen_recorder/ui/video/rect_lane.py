"""RectLane — 사각형 효과의 lane 자식 클래스.

ArrowLane 패턴 답습 — 막대 그리기 + 좌클릭 선택 + 좌우 드래그(시간 이동) +
양 끝 드래그(길이 조정) + Delete 키 삭제.
"""
...
_BAR_RADIUS = 3
_EDGE_HANDLE_PX = 5
_BAR_BG = QColor(34, 211, 238, 180)            # 청록 + 알파
_BAR_BG_SELECTED = QColor(103, 232, 249, 230)
_BAR_BORDER = QColor(0, 0, 0, 220)
_BAR_BORDER_SELECTED = QColor(255, 255, 255, 230)
_TEXT_COLOR = QColor(15, 23, 42, 235)


class RectLane(EffectLane):
    """사각형 효과 lane — 막대 그리기·드래그·삭제."""
    ...
    def _bar_rect_for(self, eff: RectEffect):
        ...
        for eff in self._effects:
            ...
            if x2 - x1 > 24:
                ...
                p.drawText(x1 + 4, row_top, x2 - x1 - 8, self.TRACK_ROW_HEIGHT,
                           Qt.AlignVCenter | Qt.AlignLeft, "▭ 사각형")
