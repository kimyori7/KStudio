"""ZoomLane — zoom 효과의 lane 자식 클래스.

CaptionLane / SpeedLane 패턴을 답습 — 막대 그리기 + ⊕ N× 라벨 + 좌클릭 선택 +
좌우 드래그(시간 이동) + 양 끝 드래그(길이 조정) + Delete 키 삭제.

paint 색은 _TYPE_COLOR["zoom"] 의 초록 계열 + 알파 (effect_lanes_widget._TYPE_COLOR
와 일관 — 헤더 색과 막대 색이 시각적으로 매칭).

v1: 정적 줌 — 라벨은 eff.start.scale 만 표시.
"""
...
_BAR_RADIUS = 3
_EDGE_HANDLE_PX = 5
# effect_lanes_widget._TYPE_COLOR["zoom"] = "#10b981" (초록). RGB(16, 185, 129).
_BAR_BG = QColor(16, 185, 129, 180)
_BAR_BG_SELECTED = QColor(52, 211, 153, 230)
_BAR_BORDER = QColor(0, 0, 0, 220)
_BAR_BORDER_SELECTED = QColor(255, 255, 255, 230)
_TEXT_COLOR = QColor(255, 255, 255, 235)


class ZoomLane(EffectLane):
    """줌 효과 lane — 막대 그리기·드래그·삭제."""
    ...
    # ---------- helpers ----------
    def _bar_rect_for(self, eff: ZoomEffect):
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        for eff in self._effects:
            ...
            if x2 - x1 > 24:
                ...
                # v1: 정적 줌 — start.scale 만 표시. 정수면 '2×', 소수면 '1.5×' (':g').
