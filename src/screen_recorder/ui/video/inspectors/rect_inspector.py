"""RectInspector — 사각형 효과 편집 폼.

ArrowInspector 와 동일 패턴 — _emitting_guard, dataclasses.replace,
effect_changed / effect_deleted 시그널.

스핀:
- start.x / start.y, end.x / end.y — 0~1 정규화 대각 모서리
- thickness — 1~64 px (테두리 굵기, source 단위)
...
class RectInspector(InspectorBase):
    """사각형 효과 편집 폼."""
    ...
    def __init__(self) -> None:
        ...
        self._effect: Optional[RectEffect] = None
    ...
    def _build_ui(self) -> None:
        ...
        form.addRow("테두리 굵기", self._thickness_spin)
        ...
        form.addRow("모서리1 X (0~1)", self._start_x_spin)
        self._start_y_spin = self._mk_norm_spin()
        form.addRow("모서리1 Y (0~1)", self._start_y_spin)
        self._end_x_spin = self._mk_norm_spin()
        form.addRow("모서리2 X (0~1)", self._end_x_spin)
        self._end_y_spin = self._mk_norm_spin()
        form.addRow("모서리2 Y (0~1)", self._end_y_spin)

        self._delete_btn = QPushButton("이 사각형 삭제")
    ...
    def set_effect(self, effect: Optional[RectEffect]) -> None:
        super().set_effect(effect)
        if effect is None:
            self._effect = None
            self._set_form_enabled(False)
            return
        if not isinstance(effect, RectEffect):
            return
    ...
    def load_effect(self, effect: Optional[RectEffect]) -> None:
    ...
    def _on_color_clicked(self) -> None:
        if self._effect is None:
            return
        chosen = QColorDialog.getColor(QColor(self._color), self, "사각형 색")
    ...
    def _on_any_change(self, *_) -> None:
        if self._emitting_guard or self._effect is None:
            return
        from ....effects.types.rect import Fade
