"""RectInspector — 폼 값 ↔ RectEffect 양방향 (ArrowInspector 패턴)."""
from __future__ import annotations

import pytest

from screen_recorder.effects.types.rect import RectEffect, Point, Fade
from screen_recorder.ui.video.inspectors.rect_inspector import RectInspector
...
def _make_rect(**kw) -> RectEffect:
    return RectEffect(in_ms=kw.pop("in_ms", 1000), out_ms=kw.pop("out_ms", 4000), **kw)
...
def test_fade_change_emits(inspector):
    eff = _make_rect()
    inspector.set_effect(eff)
    received: list = []
    inspector.effect_changed.connect(received.append)
    inspector._fade_in_spin.setValue(500)
...
def test_no_signal_during_set_effect(inspector):
    received: list = []
    inspector.effect_changed.connect(received.append)
    inspector.set_effect(_make_rect(thickness=8))
    assert received == []
